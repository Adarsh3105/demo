...
with open("WHO.json.txt") as file:				 
	data = json.load(file) 
...
inputdata = tflearn.input_data(shape=[None, len(training[0])]) 
l1 = tflearn.fully_connected(inputdata, 8) 
l2 = tflearn.fully_connected(l1, 8) 
l3 = tflearn.fully_connected(l2, len(output[0]), activation="softmax") 
outputdata = tflearn.regression(l3) 


# remove comment to not train model after you satisfied with the accuracy 
model = tflearn.DNN(outputdata) 
try:							 
...
def chat(inp): 
	...
	while True: 
		...
		if results[results_index] > 0.7: 
			...
			return ("I am sorry but I can't understand") 
